...
export default function AdminMenuEditor(){
  ...
  const [draft, setDraft] = useState({
    name: '', category: 'Frappe', desc: '',
    regular: '', tall: '', venti: '', slice: '', whole: ''
  })
  ...
  useEffect(()=>{
    localStorage.setItem('vb-menu', JSON.stringify(items))
  },[items])
  ...
  function addItem(){
    ...
    const prices = {}
    const r = cleanNumber(draft.regular)
    const t = cleanNumber(draft.tall)
    const v = cleanNumber(draft.venti)
    const s = cleanNumber(draft.slice)
    const w = cleanNumber(draft.whole)
    if (draft.category === 'Food') {
      if (s !== undefined) prices.Slice = s
      if (w !== undefined) prices.Whole = w
    } else {
      if (r !== undefined) prices.Regular = r
      if (t !== undefined) prices.Tall = t
      if (v !== undefined) prices.Venti = v
    }
    const item = {
  ...
  function resetToRepo(){
    if(confirm('Reset to the version in the repo? This will overwrite your current edits.')){
  ...
  function updateItem(idx, field, value){
    setItems(prev=>{
      const copy = [...prev]
      const it = {...copy[idx]}
      if (field === 'name' || field === 'desc' || field === 'category'){
        it[field] = value
      } else {
        // price fields like Regular/Tall/Venti/Slice/Whole
        it.prices = {...(it.prices||{})}
        const n = cleanNumber(value)
        if (n === undefined || value === '') delete it.prices[field]
        else it.prices[field] = n
      }
      copy[idx] = it
      return copy
    })
  ...
  const sizeFields = (cat, values, setValues) => (
    <>
      {cat === 'Food' ? (
  ...
  return (
    <main className="section">
      <div className="container">
        <h2 style={{margin:'0 0 6px'}}>Admin — Menu Editor</h2>
        <div className="badge">This page is not linked in the navbar. Visit /admin to use it.</div>
        ...
        <div className="card" style={{marginTop:12}}>
          ...
          <div style={{display:'grid', gap:8, marginTop:8}}>
            ...
            {sizeFields(draft.category, draft, setDraft)}
        ...
        {items.length === 0 && (
          <div className="card" style={{marginTop:12}}>No items yet. Add your first item above.</div>
        )}
      </div>
    </main>
  )
