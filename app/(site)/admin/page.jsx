...
export default function AdminMenuEditor(){
  ...
  function removeItem(idx){ const name = items[idx]?.name||'this item'; if(confirm(`Delete "${name}"?`)) setItems(prev=>prev.filter((_,i)=>i!==idx)) }
  function downloadJSON(){
    const blob = new Blob([JSON.stringify(items,null,2)], {type:'application/json'})
    const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href=url; a.download='menu.json'; a.click(); URL.revokeObjectURL(url)
  }
  function resetToRepo(){ if(confirm('Reset to the version in the repo?')) setItems(initialMenu) }
  function updateItem(idx, field, value){
    setItems(prev=>{ const copy=[...prev]; const it={...(copy[idx]||{})}
  ...
  return (
    <main className="section">
      <div className="container">
        ...
        <div className="badge">This page is hidden from the navbar. Visit /admin to use it.</div>
